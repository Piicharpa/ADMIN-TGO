...
const VerifierApp = () => {
        ...
        switch (tabIndex) {
          ...
          case 2: // ได้รับการพิจารณาแล้ว (Successful)
            filteredData = data.filter(
              (v: Verifier) => v.status === "successful"
            );
            break;
          default: // ทั้งหมด (All)
            break;
  ...
  const filteredVerifiers = (verifiers ?? []).filter((verifier) =>
    verifier.name.toLowerCase().includes(nameFilter.toLowerCase())
  );
  ...
  const deleteSelectedVerifiers = () => {
    if (selectedVerifiers) {
      Promise.all(
        selectedVerifiers.map((v) =>
          VerifierService.deleteVerifier(v.auditor_id)
        )
      )
        .then(() => {
          fetchData(activeIndex);
  ...
  const deleteVerifierDialogFooter = (
    <>
      <Button
        ...
        icon="pi pi-times"
        text
        onClick={hideDeleteVerifierDialog}
      />
  ...
  const deleteVerifiersDialogFooter = (
    <>
      <Button
        ...
        icon="pi pi-times"
        text
        onClick={hideDeleteVerifiersDialog}
      />
      <Button
        label="ใช่"
        icon="pi pi-check"
        text
        onClick={deleteSelectedVerifiers}
      />
    </>
  );
      ...
      <div className="col-12">
        <div className="card">
          ...
          <div className="flex justify-content-between align-items-center mb-4">
            ...
            <div className="flex align-items-center">
              <span className="p-input-icon-left mr-3">
                ...
                <InputText
                  type="search"
                  placeholder="ค้นหาด้วยชื่อผู้ทวนสอบ..."
                  value={nameFilter}
          ...
          <DataTable
            ...
            onSelectionChange={(e) =>
              setSelectedVerifiers(e.value as Verifier[])
            }
            dataKey="auditor_id"
            paginator
            rows={10}
          ...
          >
            ...
            <Column
              ...
              sortable
              body={registerIdBodyTemplate}
              headerStyle={{ minWidth: "15rem" }}
            ></Column>
            <Column
              field="registration_date"
              header="วันที่ลงทะเบียน"
              body={registrationDateBodyTemplate}
              sortable
            ></Column>
            <Column
          ...
          >
            ...
            <div className="field">
              ...
              />
              {submitted && !verifier.register_id && (
                <small className="p-error">
                  จำเป็นต้องกรอกรหัสการลงทะเบียน
                </small>
              )}
            </div>
            <div className="field">
          ...
          <Dialog
            ...
            header="ยืนยัน"
            modal
            footer={deleteVerifierDialogFooter}
            onHide={hideDeleteVerifierDialog}
          >
          ...
          <Dialog
            ...
            header="ยืนยัน"
            modal
            footer={deleteVerifiersDialogFooter}
            onHide={hideDeleteVerifiersDialog}
          >
            <div className="flex align-items-center justify-content-center">
              <i
                className="pi pi-exclamation-triangle mr-3"
              ...
              />
              {selectedVerifiers && (
                <span>คุณแน่ใจหรือไม่ว่าต้องการลบรายการที่เลือก?</span>
              )}
            </div>
          </Dialog>
        </div>
