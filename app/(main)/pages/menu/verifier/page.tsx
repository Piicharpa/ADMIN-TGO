...
const VerifierApp = () => {
  ...
  const fetchData = (tabIndex: number) => {
    ...
    VerifierService.getVerifiers()
      ...
      .catch(() => {
        toast.current?.show({ severity: "error", summary: "Error", detail: "Cannot fetch data" });
      })
  ...
  const saveVerifier = () => {
    ...
    service
      ...
      .catch(() => toast.current?.show({ severity: "error", summary: "เกิดข้อผิดพลาด" }));
  };
  ...
  return (
    <div className="card">
      ...
      <div className="flex justify-content-between mb-4">
        <h5>ผู้ทวนสอบ</h5>
        <InputText placeholder="ค้นหาชื่อ..." value={nameFilter} onChange={(e) => setNameFilter(e.target.value)} />
      </div>

      <SelectButton value={activeIndex} onChange={(e) => setActiveIndex(e.value)} options={tabOptions} optionLabel="label" className="mb-4" />

      <DataTable value={filteredVerifiers} selection={selectedVerifiers} onSelectionChange={(e) => setSelectedVerifiers(e.value as Verifier[] | null)} dataKey="auditor_id" paginator rows={10} loading={loading}>
      ...
      <AddEditDialog
        ...
        onSave={saveVerifier}
        onChange={(e, name) => setVerifier({ ...verifier, [name]: "value" in e ? e.value : e.target.value })}
      />

      {/* View Dialog */}
      <Dialog visible={viewDialog} header="รายละเอียดผู้ทวนสอบ" modal onHide={() => setViewDialog(false)}>
        <p><b>รหัสผู้ทวนสอบ:</b> {verifier.auditor_id}</p>
        <p><b>ชื่อ:</b> {verifier.name}</p>
        <p><b>รหัสลงทะเบียน:</b> {verifier.register_id}</p>
        <p><b>วันที่ลงทะเบียน:</b> {verifier.registration_date}</p>
        <p><b>อีเมล:</b> {verifier.description}</p>
        <p><b>ความเชี่ยวชาญ:</b> {verifier.expertise}</p>
        <p><b>องค์กร:</b> {verifier.organization}</p>
        <p><b>ที่อยู่:</b> {verifier.address}</p>
      </Dialog>

      {/* Delete Dialog */}
      <DeleteDialog visible={deleteVerifierDialog} onHide={() => setDeleteVerifierDialog(false)} onConfirm={deleteVerifier} message={`คุณแน่ใจว่าจะลบ ${verifier.name}?`} />
      <DeleteDialog visible={deleteVerifiersDialog} onHide={() => setDeleteVerifiersDialog(false)} onConfirm={deleteSelectedVerifiers} message={`คุณแน่ใจว่าจะลบ ${selectedVerifiers?.length} รายการ?`} />
    </div>
  );
};
